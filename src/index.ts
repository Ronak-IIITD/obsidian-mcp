...
if (!VAULT_PATH) {
  console.error("[obsidian-mcp] ERROR: VAULT_PATH environment variable is required.");
  process.exit(1);
}
...
function resolvePath(notePath: string): string {
  ...
  if (!resolved.startsWith(vaultPath)) throw new Error(`Path "${notePath}" is outside the vault.`);
  return resolved;
}
...
function buildFrontmatter(tags: string[], project: string | undefined): string {
  ...
  if (tags.length > 0) { lines.push("tags:"); tags.forEach(t => lines.push(`  - ${t.replace(/^#/, "")}`)); }
  lines.push("---", "");
  return lines.join("\n");
}
...
async function listMdFiles(dir: string): Promise<string[]> {
  ...
  let entries: string[];
  try { entries = (await fs.readdir(dir)) as string[]; } catch { return results; }
...
async function fileContains(filePath: string, query: string): Promise<boolean> {
  try { return (await fs.readFile(filePath, "utf-8")).toLowerCase().includes(query.toLowerCase()); }
  catch { return false; }
}
...
server.tool(
  ...
  "Create a new note in the Obsidian vault. Use when the user wants to save a learning, insight, decision, summary, or any content.",
  {
    ...
    folder: z.string().optional().describe('Subfolder inside the vault, e.g. "Projects/MyApp"'),
    tags: z.array(z.string()).optional().describe('Tags, e.g. ["dev", "learning"]'),
    project: z.string().optional().describe("Project name for frontmatter"),
    overwrite: z.boolean().optional().describe("Overwrite if note exists. Defaults to false."),
  },
  async ({ title, content, folder, tags = [], project, overwrite = false }) => {
    ...
    if (await fileExists(fullPath) && !overwrite) {
      return { content: [{ type: "text" as const, text: `Note "${filename}" already exists. Set overwrite: true to replace it, or use append_note to add to it.` }] };
    }
    await fs.writeFile(fullPath, buildFrontmatter(tags, project) + `# ${title}\n\n${content}\n`, "utf-8");
    return { content: [{ type: "text" as const, text: `✅ Note saved: ${path.relative(vaultPath, fullPath)}` }] };
  }
);
...
server.tool(
  ...
  {
    path: z.string().describe('Relative path to the note, e.g. "Projects/MyApp.md"'),
    content: z.string().describe("Markdown content to append"),
    heading: z.string().optional().describe('Optional heading, e.g. "## 2026-03-17 Update"'),
  },
  async ({ path: notePath, content, heading }) => {
    const fullPath = resolvePath(ensureMd(notePath));
    if (!await fileExists(fullPath)) {
      return { content: [{ type: "text" as const, text: `Note not found: "${notePath}". Use save_note to create it first.` }] };
    }
    const timestamp = new Date().toISOString().split("T")[0];
    const section = heading ? `\n\n${heading}\n\n${content}` : `\n\n---\n*Updated: ${timestamp}*\n\n${content}`;
    await fs.appendFile(fullPath, section, "utf-8");
    return { content: [{ type: "text" as const, text: `✅ Appended to: ${path.relative(vaultPath, fullPath)}` }] };
  }
);
...
server.tool(
  ...
  async ({ path: notePath }) => {
    const fullPath = resolvePath(ensureMd(notePath));
    try {
      return { content: [{ type: "text" as const, text: await fs.readFile(fullPath, "utf-8") }] };
    } catch {
...
server.tool(
  ...
  "Search for notes in the Obsidian vault by keyword. Searches both filenames and file content.",
  {
    ...
    folder: z.string().optional().describe('Limit search to a subfolder, e.g. "Projects"'),
  ...
  async ({ query, folder, limit = 10 }) => {
    ...
    for (const file of allFiles) {
      ...
      if (relative.toLowerCase().includes(query.toLowerCase()) || await fileContains(file, query)) {
...
server.tool(
  ...
  { folder: z.string().optional().describe('Subfolder to list, e.g. "Projects". Defaults to vault root.') },
  async ({ folder }) => {
    const searchRoot = folder ? path.join(vaultPath, folder) : vaultPath;
    const files = await listMdFiles(searchRoot);
    if (files.length === 0) {
      return { content: [{ type: "text" as const, text: folder ? `No notes found in "${folder}"` : "Vault appears to be empty." }] };
    }
    const relative = files.map(f => path.relative(vaultPath, f)).sort();
    return { content: [{ type: "text" as const, text: `${relative.length} note(s):\n\n${relative.map(f => `- ${f}`).join("\n")}` }] };
  }
);
...
server.tool(
  ...
  {
    path: z.string().describe('Relative path to the note, e.g. "Projects/OldNote.md"'),
    confirm: z.boolean().describe("Must be true to confirm deletion."),
  },
  async ({ path: notePath, confirm }) => {
    if (!confirm) return { content: [{ type: "text" as const, text: `Deletion cancelled. Set confirm: true to actually delete "${notePath}".` }] };
    const fullPath = resolvePath(ensureMd(notePath));
    try { await fs.unlink(fullPath); return { content: [{ type: "text" as const, text: `🗑️ Deleted: ${notePath}` }] }; }
    catch { return { content: [{ type: "text" as const, text: `Note not found: "${notePath}"` }] }; }
  }
);
...
server.tool(
  ...
  "Append content to today's daily note (creates it if missing). Great for logging project progress, quick thoughts, or build logs.",
  {
    ...
    section: z.string().optional().describe('Section heading, e.g. "## OpenCode Session"'),
    folder: z.string().optional().describe('Folder for daily notes. Defaults to "Daily Notes".'),
  },
  async ({ content, section, folder = "Daily Notes" }) => {
    ...
    if (!await fileExists(fullPath)) {
      await fs.writeFile(fullPath, buildFrontmatter(["daily"], undefined) + `# ${today}\n`, "utf-8");
    }
    const heading = section ?? `## ${new Date().toLocaleTimeString()}`;
    await fs.appendFile(fullPath, `\n\n${heading}\n\n${content}\n`, "utf-8");
    return { content: [{ type: "text" as const, text: `✅ Logged to daily note: ${path.relative(vaultPath, fullPath)}` }] };
  }
);

async function main() {
  try { await fs.access(vaultPath); }
  catch {
    console.error(`[obsidian-mcp] ERROR: Vault path does not exist: "${vaultPath}"`);
    process.exit(1);
  }
...
main().catch(err => { console.error("[obsidian-mcp] Fatal error:", err); process.exit(1); });